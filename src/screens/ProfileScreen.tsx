...
export function ProfileScreen() {
  ...
  const [settings, setSettings] = useState(() => {
    ...
    return {
      language: parsed.language ?? true,
      aiTranslation: parsed.aiTranslation ?? false,
      classReminders: parsed.classReminders ?? true,
  ...
  const toggleSetting = async (key: keyof typeof settings) => {
    ...
    if ((key === 'classReminders' || key === 'deadlineReminders') && newValue) {
    ...
    <div className="min-h-screen bg-transparent dark:bg-transparent pb-24 transition-colors">
        ...
        <section>
           ...
           <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-2 space-y-1 shadow-sm border border-slate-50 dark:border-slate-800 transition-colors">
              ...
              <ToggleItem 
                ...
                subtext="Reminds 1 day before CT / Assignment"
                active={settings.deadlineReminders} 
      ...
      {isEditing && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-4 bg-slate-900/40 backdrop-blur-sm overflow-y-auto pt-10 pb-10">
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-6 w-full max-w-md shadow-2xl border border-slate-100 dark:border-slate-800 relative my-auto animate-in fade-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto no-scrollbar">
            <h3 className="text-xl font-black text-slate-800 dark:text-slate-100 text-center mb-6">Edit Profile</h3>
            ...
            <div className="flex flex-col items-center mb-6">
              <div className="relative">
                {editData.avatarUrl ? (
                  <img 
                    ...
                    alt="Upload Preview" 
                    className="w-24 h-24 rounded-[2rem] border-4 border-cyan-500 shadow-xl object-cover"
                    onError={(e) => {
                      const target = e.target as HTMLImageElement;
                      target.onerror = null;
                      target.src = '/assets/images/profile.png';
                  ...
                  <div className="w-24 h-24 rounded-[2rem] bg-gradient-to-br from-cyan-400 to-blue-500 border-4 border-white dark:border-slate-800 shadow-xl flex items-center justify-center text-white font-bold text-3xl">
              ...
              {imageError ? (
                <p className="text-[10px] text-rose-500 font-bold mt-1 text-center">{imageError}</p>
              ) : (
                <p className="text-[9px] text-slate-400 dark:text-slate-500 mt-1 text-center font-medium">JPEG or PNG under 2MB.</p>
              )}
            </div>
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-500 block mb-0.5">Full Name</label>
                <input 
                  type="text"
                  required
                  className="w-full text-xs font-bold bg-[#f8fafc] dark:bg-slate-800 p-3.5 rounded-2xl border border-transparent dark:border-slate-700 focus:bg-white dark:focus:bg-slate-900 focus:border-cyan-500 dark:focus:border-cyan-500 outline-none text-slate-800 dark:text-white transition-all shadow-sm"
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-500 block mb-0.5">Student ID</label>
                <input 
                  type="text"
                  required
                  className="w-full text-xs font-bold bg-[#f8fafc] dark:bg-slate-800 p-3.5 rounded-2xl border border-transparent dark:border-slate-700 focus:bg-white dark:focus:bg-slate-900 focus:border-cyan-500 dark:focus:border-cyan-500 outline-none text-slate-800 dark:text-white transition-all shadow-sm"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-500 block mb-0.5">Batch</label>
                  <select
                    className="w-full text-xs font-bold bg-[#f8fafc] dark:bg-slate-800 p-3.5 rounded-2xl border border-transparent dark:border-slate-700 focus:bg-white dark:focus:bg-slate-900 focus:border-cyan-500 dark:focus:border-cyan-500 outline-none text-slate-800 dark:text-white transition-all shadow-sm"
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-500 block mb-0.5">Semester</label>
                  <select
                    className="w-full text-xs font-bold bg-[#f8fafc] dark:bg-slate-800 p-3.5 rounded-2xl border border-transparent dark:border-slate-700 focus:bg-white dark:focus:bg-slate-900 focus:border-cyan-500 dark:focus:border-cyan-500 outline-none text-slate-800 dark:text-white transition-all shadow-sm"
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-500 block mb-0.5">Section</label>
                <select
                  className="w-full text-xs font-bold bg-[#f8fafc] dark:bg-slate-800 p-3.5 rounded-2xl border border-transparent dark:border-slate-700 focus:bg-white dark:focus:bg-slate-900 focus:border-cyan-500 dark:focus:border-cyan-500 outline-none text-slate-800 dark:text-white transition-all shadow-sm"
