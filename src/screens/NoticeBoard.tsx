...
export function NoticeBoard({ onBack }: NoticeBoardProps) {
  ...
  const [newAssignment, setNewAssignment] = useState({ title: '', course: '', deadline: '', type: 'assignment' });
                   ...
                   <div className="flex items-start justify-between mb-4">
                      <div className="flex-1">
                         ...
                         <div className="flex items-center gap-2 mb-3">
                           <span className="text-[10px] font-bold text-indigo-500 uppercase tracking-widest">{asgn.course}</span>
                           <span className={`px-2 py-0.5 rounded-md text-[8px] font-black uppercase tracking-wider ${
                             asgn.type === 'ct' 
                               ? 'bg-rose-50 dark:bg-rose-950/30 text-rose-500 border border-rose-100 dark:border-rose-900/20' 
                               : 'bg-indigo-50 dark:bg-indigo-950/30 text-indigo-500 border border-indigo-100 dark:border-indigo-900/20'
                           }`}>
                             {asgn.type === 'ct' ? 'Class Test' : 'Assignment'}
                           </span>
                         </div>
                   ...
                   <div className="space-y-4">
                      <div className="space-y-2">
                         ...
                         <div className="grid grid-cols-2 gap-2 bg-slate-50 dark:bg-slate-800 p-1 rounded-2xl">
                            <button
                               type="button"
                               onClick={() => setNewAssignment({...newAssignment, type: 'assignment'})}
                            ...
                            <button
                               type="button"
                               onClick={() => setNewAssignment({...newAssignment, type: 'ct'})}
                               className={`py-3.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
                                  newAssignment.type === 'ct'
                                     ? 'bg-white dark:bg-slate-700 text-slate-800 dark:text-white shadow-sm'
                                     : 'text-slate-400 dark:text-slate-500 hover:text-slate-600'
                               }`}
                            >
