...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const registrationDetailsRef = useRef<any>(null);

  useEffect(() => {
    let unsubProfile: (() => void) | null = null;

    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      // Unsubscribe standard profile listener if user auth state changes
      if (unsubProfile) {
        unsubProfile();
        unsubProfile = null;
      }
      ...
      if (firebaseUser) {
        ...
        unsubProfile = onSnapshot(userRef, async (docSnap) => {
          if (docSnap.exists()) {
            ...
            const fallbackProfile = registrationDetailsRef.current || {
  ...
  const registerWithEmail = async (email: string, pass: string, name: string, studentId: string, batch: string, section: string, semester: string) => {
    ...
    try {
      ...
      const initialRoles: UserRole[] = ['Student'];
      registrationDetailsRef.current = {
        fullName: name.trim(),
        email: cleanEmail,
        roles: initialRoles,
        studentId: cleanId,
      ...
      try {
        ...
        console.warn("Auth Display Name update failed (non-critical):", profileError);
      }
      ...
      try {
        ...
        // CLEANUP: If database write fails during checkout/registration, delete the auth user
        // to prevent inconsistent auth-only accounts that trigger 'email-already-in-use' errors.
        try {
          await result.user.delete();
        } catch (authDeleteError) {
          console.error("Cleanup: Failed to remove auth user after Firestore write error:", authDeleteError);
        }
  ...
  const logout = async () => {
    registrationDetailsRef.current = null;
    await signOut(auth);
  };
